...
// Filter audioOutput events from logs
...
/**
 * Run two-turn canary test
 * @param {Object} config - Configuration object
 * @param {Function} config.invokeAgent - Async function to start agent session
 *   Should accept: { sessionId, userId, systemPrompt, voiceId, mcpConfig }
 *   Should return: { success: boolean, error?: string }
 * @param {string} config.eventApiEndpoint - AppSync Events endpoint
 * @param {string} config.eventBusNamespace - Event bus namespace
 * @param {string} config.awsRegion - AWS region
 * @param {string} config.audioFile1 - Path to turn 1 audio file
 * @param {string} config.audioFile2 - Path to turn 2 audio file
 * @param {string} [config.recordingDir] - Directory to save recordings (optional)
 */
async function runTwoTurnCanary(config) {
  const {
    invokeAgent,
    eventApiEndpoint,
    eventBusNamespace,
  ...
  try {
    if (!fs.existsSync(audioFile1)) {
      throw new Error(`Test audio not found at ${audioFile1}`);
    }
    if (!fs.existsSync(audioFile2)) {
      throw new Error(`Test audio not found at ${audioFile2}`);
    }
    
    const audioChunks1 = loadAudioChunks(audioFile1);
    const audioChunks2 = loadAudioChunks(audioFile2);
    
    console.log(`✓ Loaded turn 1 audio: ${path.basename(audioFile1)} (${audioChunks1.length} chunks)`);
    console.log(`✓ Loaded turn 2 audio: ${path.basename(audioFile2)} (${audioChunks2.length} chunks)`);
    ...
    Amplify.configure(
      {
        API: {
          Events: {
            endpoint: `${eventApiEndpoint}/event`,
            region: awsRegion,
            defaultAuthMode: 'iam',
          },
        },
    ...
    const channelPath = `/${eventBusNamespace}/user/${userId}/${sessionId}`;
    ...
    if (!invokeResult.success) {
      throw new Error(`Failed to start session: ${invokeResult.error}`);
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      const finishTest = async () => {
        ...
        if (recordingDir) {
          ...
          const dir = path.join(recordingDir, `${timestamp}-${testId}`);
          if (!fs.existsSync(dir)) {
            fs.mkdirSync(dir, { recursive: true });
          }
          
          if (turn1InputAudioChunks.length > 0) {
            saveAudioToWav(turn1InputAudioChunks, path.join(dir, 'turn1-input.wav'), 16000);
            console.log(`\n💾 Turn 1 input audio saved`);
          }
          if (turn1AudioChunks.length > 0) {
            saveAudioToWav(turn1AudioChunks, path.join(dir, 'turn1-output.wav'), 24000);
            console.log(`💾 Turn 1 output audio saved`);
          }
          if (turn2InputAudioChunks.length > 0) {
            saveAudioToWav(turn2InputAudioChunks, path.join(dir, 'turn2-input.wav'), 16000);
            console.log(`💾 Turn 2 input audio saved`);
          }
          if (turn2AudioChunks.length > 0) {
            saveAudioToWav(turn2AudioChunks, path.join(dir, 'turn2-output.wav'), 24000);
            console.log(`💾 Turn 2 output audio saved`);
          }
        ...
        next: async (data) => {
          try {
            ...
            if (event.event === 'audioOutput') {
              let totalBytes = 0;
              const outputSampleRate = event.data?.sampleRate || 24000;
...
module.exports = { runTwoTurnCanary, publishMetrics };
