...
# Health check errors should now be resolved with FastAPI handling both HTTP and WebSocket properly
...
async def websocket_endpoint(websocket: WebSocket):
    """WebSocket handler using FastAPI"""
    await websocket.accept()
    
    print(f"[AGENT] Client connected from {websocket.client}")
    session = None
    ...
    try:
        ...
        print("[AGENT] Starting response forwarding task...")
        forward_task = asyncio.create_task(
            forward_bedrock_to_fastapi_client(websocket, session)
        )
...
async def forward_bedrock_to_fastapi_client(websocket: WebSocket, session: S2sSessionManager):
    """Forward Bedrock responses to FastAPI WebSocket client"""
    print("[FORWARD] Starting to forward Bedrock responses...")
    audio_ended = False
    ...
    try:
        while session.is_active:
            ...
            try:
                ...
                print(f"[FORWARD] Error sending to client: {e}")
                break
...
async def run_server(host='0.0.0.0', port=None):
    """Run FastAPI server with WebSocket support and health checks"""
    port = port or int(os.environ.get('PORT', 9000))
    
    print(f"Starting Nova Sonic WebSocket agent on ws://{host}:{port}")
    print(f"[AGENT] FastAPI server ready on http://{host}:{port}")
    print(f"[AGENT] WebSocket endpoint ready on ws://{host}:{port}")
    print(f"[AGENT] Health check ready on http://{host}:{port}/health")
...
def handle_exception(loop, context):
    ...
    # TODO: Fix ALB health check properly - see warning filter above
